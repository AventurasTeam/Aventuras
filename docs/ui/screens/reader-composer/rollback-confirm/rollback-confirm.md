...
| **K world-state changes** | All other narrative deltas in range (entities, lore, threads, happenings, entry_assets). Every `story_entries` delta is excluded, not just the creates: a non-create one amends an entry the sweep is deleting, which the entries line already reports. The discriminator is the target table, not `entry_id` — a delta on another table stays counted whether or not it carries an anchor. |
