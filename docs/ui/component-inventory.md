...
| EmbedWindowTextarea        | `components/compounds/` | `Textarea` plus a token counter for the embedder's input window, shown from 75% and warning past 100% — text past the window is dropped at embed time and cannot be retrieved. Counts through the local model's own tokenizer (exact) or an estimator for a provider (marked approximate). The counter is always mounted and only its opacity moves: conditionally rendering it would remount the `TextInput` mid-entry per [input-adornment-dom-identity](../implementation/lessons-learned/input-adornment-dom-identity.md). Spec: [retrieval.md → What gets embedded per type](../memory/retrieval.md#what-gets-embedded-per-type).                                                |
