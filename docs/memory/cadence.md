...
| `classifierCadence` (entries)        | When the periodic classifier runs in the background.                                                                                                                                                                    | UI warns in partial mode when the effective cadence exceeds `partialChapterBuffer` (unclassified entries slide out of the window before classifier catches up); an odd cadence counts as the next whole turn — see Buffer-aware cadence indicator below. Suppressed in full mode. |
