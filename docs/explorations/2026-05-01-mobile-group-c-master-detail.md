...
1. **Tab strip** — the desktop primitive. Used on desktop at every
   count, and on tablet when count ≤ 5.
...
3. **Select dropdown** — collapsed picker chip. Used on phone when
   count ≥ 3, and on tablet when count > 5.

**The consumer's decision is binary.** Either render Tab strip
(desktop always; tablet when count ≤ 5), or hand the tab list to
...
| Tier    | Tab count ≤ 2  | Tab count 3–5   | Tab count > 5   |
| ------- | -------------- | --------------- | --------------- |
...
**Threshold of 5 on tablet** falls cleanly between 4 (happenings)
and 7 (location / item / faction), so no kind sits on the
boundary. **Threshold of 2 on phone** is the Select primitive's
...
**Load-bearing assumption.** The 5-tab threshold for tab strip vs
Select dropdown assumes label widths cluster around ~80–90 px each
("Overview", "Identity", "Carrying", "Connections", "Settings",
"Assets", "Involvements", "History"). Verified by counting label
char widths — longest is "Involvements" (12 chars, ~96 px at the
detail-pane font scale). 5 × 96 = 480 px; iPad portrait detail
pane (~430 px after 340 px list pane) fits 4 comfortably, 5
borderline. The 5-threshold rule routes 5-tab kinds to dropdown on
tablet — correct call. If a future kind has 5 short labels (3-4
chars each), the strip might still fit on tablet; rule errs on the
side of dropdown, which is consistent and acceptable.
...
- **Tablet detail-pane width at the boundary.** iPad portrait
  ...
  desktop tier. The 5-threshold lands characters/locations/items/
...
- **Assumed.** Tab label widths cluster around 80–96 px per label
  ...
  font scale differs and labels render wider, the 5-threshold rule
