...
- **`buildGenerationContext` should read its own data — an
  identity-only builder sourced from SQLite.** Target shape: the
  builder takes run identity and run-scoped flags and nothing else,
  reading what it needs from the DB. Call sites pass `ctx` plus
  booleans; `entries`, `entities`, `definition` and `settings` all
  leave the signature, and `idMap`, `retrieval` and `refreshGuidance`
  come off `ctx.intermediates` / `ctx.inputs` as
  [`architecture.md → The single-context principle`](./architecture.md#the-single-context-principle)
  already prescribes for run-scoped values. The driver is
  **enforcement, not deduplication**: for as long as the builder
  accepts domain data, any new phase can hand the group a bespoke
  slice and the unified context stops being unified with nothing
  failing. All four removed arguments are pure store pass-through
  today, so no caller loses anything it computes itself.

  Two correctness drivers, both verified against the code and the real
  `composePromptBuffer` on 2026-08-28. Both are **latent, not live**:
  the three buffer knobs have no settings editor yet, and the defaults
  (`partialChapterBuffer` 10, `protectedBuffer` 10,
  `fullChapterInBuffer` false) stay inside every bound below. A
  buffer-settings surface arms both, so this item should land before
  one ships.
  - **The prompt buffer is capped by the reader's window, and its real
    size depends on scroll history.** `entriesStore` hydrates with the
    last `ENTRIES_WINDOW_SIZE` (50) entries; scroll-up paging adds a
    page at a time without evicting, and the reader's `reload` drops
    those again. `composePromptBuffer` runs over whatever that leaves,
    so `fullChapterInBuffer` against a 200-entry open chapter silently
    yields about fifty, and two generations in one session can compose
    different buffers depending only on whether the reader scrolled
    between them. Nothing declares the cap. The periodic classifier
    already routes around it by reading SQLite directly and says why
    in `lib/pipeline/definitions/periodic-classifier.ts`; the per-turn
    path never did.
  - **The fallback classifier's two-turn window is not guaranteed.**
    Its deliberate two-entry array goes into `composePromptBuffer`
    like any other caller's, so the story's knobs cut it:
    `partialChapterBuffer` 1 with `protectedBuffer` 0 yields the tail
    alone — dropping the user action the phase exists to classify —
    and a chapter-assigned pair with `protectedBuffer` 0 yields
    nothing at all. The builder's own argument comment records the
    mechanism; what it misses is that this phase is a victim of it.

  Resulting design:
  - **`entries` stays the one budget-governed collection**, composed
    in SQL: count the open region, compute `take` with the existing
    pure arithmetic, fetch that many non-system rows by position.
    Fetches only the window instead of filtering a hydrated set.
  - **`lastTurns` replaces the caller-supplied classifier array** —
    the last two non-system entries, bounded by the query rather than
    by a caller or a template. Two rows are negligible against the
    token budget, a pack cannot widen them, and excluding system rows
    in the query closes the edge where one at either slot collapses
    the window.
  - **No general raw-entries variable.** The retrieval phase prices
    its pools against `composePromptBuffer`'s output, so a collection
    a template could render wider than `entries` would leave
    retrieval seating rows against a budget for a prompt that was
    never sent. Every entries-shaped variable is therefore either
    budget-governed or contractually small.
  - **Retrieval must share the fetch.** If the builder composes from
    SQL while the retrieval phase still prices a store-composed
    buffer, the two diverge exactly where this item is trying to fix
    them. Both call one `readPromptBuffer` helper; the `take`
    arithmetic stays a pure function so it remains unit-testable
    without a DB.
  - **The entry projection widens** to `position` and a metadata
    subset (`sceneEntities`, `currentLocationId`, `worldTime`,
    `summary`), following `PROMPT_ENTITY_FIELDS`. `EntryMetadata`
    cannot be exposed whole: it carries raw model `reasoning`, the
    reversed submission text kept for Retry, internal failure detail,
    per-call telemetry, and suggestion items for actions the story
    never took.
  - **Add a `(branchId, position)` index to `story_entries`.** The
    table has only its composite PK and a partial staleness index, so
    every ordered read sorts. Pre-existing, but this item adds a
    count and an ordered fetch per generation.

  Dissolved from the earlier record: the scene-block retargeting
  concern (an authoritative tail read makes `sceneEntities` and
  `currentLocationId` independent of any window, so nothing needs to
  become template-derived); the `recent`-filter question (the window
  lives in the query, and no bundled template uses the filter); and
  most of the ID-leak concern (omitting `entry.id` from the
  projection leaves nothing unsubstitutable, since `chapterId` is
  already covered by
  [`data-model.md → ID shape`](./data-model.md#id-shape--kind-prefixed-uuids-throughout)).

  Costs, sized 2026-08-28: the builder becomes async and gains a
  failure mode (it owns the open-story guard once it owns the reads);
  the 45 direct call sites in `generation-context.test.ts` move from
  literal arrays to `createTestDb` seeding, which is more setup per
  case but exercises the real query. Needs the clause edit to
  [`architecture.md → The single-context principle`](./architecture.md#the-single-context-principle),
  whose "a phase reads the domain stores directly" is what this item
  reverses, and a `templateContextMap` entry for `lastTurns`.
  Surfaced by M3.7a post-merge review (2026-07-30); re-verified and
  re-sized during the Slice 3.12 split (2026-08-19); re-scoped from
  "builder owns the store reads" to the DB-sourced shape after the
  window-cap findings (2026-08-28).
