...
/** Action choices matching actionChoicesResultSchema. */
const defaultActionChoicesResult = {
  choices: [
    { text: 'Draw your sword and advance', type: 'action' },
    { text: '"Who goes there?"', type: 'dialogue' },
    { text: 'Examine the glowing crystal', type: 'examine' },
  ],
}
...
describe('Adventure Mode — ActionInputController E2E', () => {
  ...
  it('full pipeline: all phases fire with rich world state', async ({ task }) => {
    ...
    interceptor
      ...
      .on('action-choices', respondWithJSON(defaultActionChoicesResult))
  ...
  // --------------------------------------------------------------------------
  // 3. Action choices disabled — skips post-generation
  // --------------------------------------------------------------------------

  it('action choices disabled: skips post-generation', async ({ task }) => {
    const testStory = buildStory({
      mode: 'adventure',
      settings: {
        pov: 'second',
