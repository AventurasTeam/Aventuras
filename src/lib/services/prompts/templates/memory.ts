...
- **aliases** — every *other* form the same subject is called by: titles, epithets, cover identities, short forms, translations. This is what makes a character findable when the scene calls them by their title. An alias identical to the name is dropped automatically; so is a keyword that repeats the name or an alias, since it can never add a match.
...
- **description** — what is true about this subject now, in plain prose. Not a chapter-by-chapter recap: the chapter summaries are already in the narrator's context, and repeating them here is paid for twice. No parenthetical glosses \`(like this one)\` and no asides correcting the record — write the fact, not the commentary on it. Describe the subject, not its neighbours: what belongs to another entry goes in that entry. When you update, rewrite the sentence that is wrong rather than appending a new one, and keep the whole thing under about 120 words.
...
{% if hasChapters %}- Use query_chapter when a summary is not enough, and ask a specific question ("What did [character] reveal?", never "Give me the full content"). Each call reads a whole chapter with a second model, so spend it deliberately.
