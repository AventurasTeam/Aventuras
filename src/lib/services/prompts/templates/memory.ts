...
**There is no story text in this session** — no chapters have been written and no story text is given. Everything you know comes from the entries themselves. Consolidate and clean them; do not create entries, and do not add facts that are not already written in an entry. An invented fact here is indistinguishable from a remembered one later.
{% endunless %}
...
{% if hasChapters %}- Use query_chapter when an earlier chapter's summary is not enough{% if hasNewChapter %} — the chapter you just wrote needs no query, since it is already given in full below{% endif %}, and ask a specific question ("What did [character] reveal?", never "Give me the full content"). Each call reads a whole chapter with a second model, there are a few per session, and asking the same question twice returns the first answer rather than reading again.
...
{% endif %}
**Consolidating is the first step, not the whole job.**{% if hasStoryMaterial %} Once the groups are closed, go back over {% if hasNewChapter %}the chapter you just wrote{% if hasChapters %}{% if hasRecentStory %}, the earlier chapter summaries and the recent story{% else %} and the earlier chapter summaries{% endif %}{% elsif hasRecentStory %} and the recent story{% endif %}{% elsif hasChapters %}the chapter summaries{% if hasRecentStory %} and the recent story{% endif %}{% else %}the recent story{% endif %}: entries the events have outdated are step 2, a subject that matters and has no entry at all is step 3. That is what this material is in front of you for, and a session that only merged duplicates has done a third of its work.{% endif %} Call finish_lore_management with a summary of what you changed when all of it is done.`,
