...
// Every entity/lore/thread/happening/chapter row below sets embedding_stale
// explicitly: none carries a real vector, and the default stays out of the read.
...
const ID = {
  ...
  vorne: 'char_vorne',
  sage: 'char_sage',
  brannoc: 'char_brannoc',
  brannocFlagged: 'char_brannoc_flagged',
  hollow: 'loc_hollow',
  market: 'loc_market',
  keep: 'loc_keep',
  blade: 'item_blade',
...
const REPLY_LINES = [
  ...
  'The broker is waiting where the alley narrows, and Vorne’s smile is the kind that has already counted your steps.',
...
const heroEntities: NewEntity[] = [
  ...
  // Collision fixture (world.md → Collision review): staged Brannoc, then a
  // classifier pass creates a namesake it couldn't promote — only the newer row is flagged.
  {
    id: ID.brannoc,
    branchId: MAIN,
    kind: 'character',
    name: 'Brannoc',
    description: 'A Watch sergeant with a debt to the Syndicate and a daughter in the Hollow.',
    status: 'staged',
    injectionMode: 'auto',
    tags: ['watch'],
    ...
    priority: 10,
    state: validState('character', {
      visual: { physique: 'broad', hair: 'grey-streaked' },
      traits: ['dutiful'],
      drives: ['keep his daughter out of it'],
      current_location_id: null,
      equipped_items: [],
      inventory: [],
      faction_id: ID.watch,
  ...
  {
    id: ID.brannocFlagged,
    branchId: MAIN,
    kind: 'character',
    name: 'Brannoc',
    description: 'A smuggler who runs the river gate after dark; nobody has seen his face.',
    status: 'active',
    injectionMode: 'auto',
    nameCollisionFlag: 1,
    tags: ['smuggler'],
    ...
    priority: 10,
    state: validState('character', {
      visual: { attire: 'oilskin hood' },
      traits: ['evasive'],
      drives: ['stay unrecognized'],
      current_location_id: ID.market,
      equipped_items: [],
      inventory: [],
      faction_id: null,
    ...
    embeddingStale: 1,
    createdAt: BASE + 12 * DAY - MIN,
    updatedAt: BASE + 12 * DAY - MIN,
  },
  {
    id: ID.hollow,
    branchId: MAIN,
  ...
  },
  {
    ...
    injectionMode: 'auto',
    tags: ['faction'],
    keywords: ['the watchman', 'watchmen'],
    priority: 15,
