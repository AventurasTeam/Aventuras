...
{% endfor %}
{% if suggestionsFire -%}
Also populate the "suggestions" field of your JSON response with exactly {{ suggestionCount }} entries, one per next-turn option. Each entry's \`text\` is COMPLETE prose written as the reader's own next turn — not a description of an option, and not a question. Categories are listed below with a bracketed ref, e.g. "[cat1] Action" — reference that ref below WITHOUT the brackets, as the \`categoryRef\` value. You may reuse a category or skip one; vary the categories across entries rather than repeating the same one:
