...
Known relationships, same ID rule — the subject's view of the object:
{%- for r in relationships %}
- [{{ r.subject }}] → [{{ r.object }}]: {{ r.kind }}
{%- endfor %}
...
- Relationships. The list above is the current state and may hold the author's own edits. Emit (subject, object, kind) only when these turns establish a view or change a listed one; never restate a listed view. subject is the character whose perspective the prose expresses. Fill ONLY the perspective the prose shows — do not infer the inverse from biology or convention. "Kael called Aria sister" records Kael's view of Aria and nothing about Aria's view of Kael.
