...
- [ ] **WSC-02**: World state passed as typed arrays: `worldStateCharacters[]`, `worldStateInventory[]`, `worldStateBeats[]`, `worldStateLocations[]` (non-current locations from tier 2/3)
- [ ] **WSC-03**: Current location already in context as `currentLocation` string — extended to include description (`currentLocationDescription`) or replaced with a full object
- [ ] **WSC-04**: Character entries include tier, relationship, description, traits, and `appearance: string[]` (TS layer normalizes `VisualDescriptors` object → flat array of non-empty values; legacy array format also normalized)
...
- [ ] **LORE-01**: `EntryRetrievalService.buildContextBlock()` deleted; `lorebookEntries[]` passed as raw Entry array with tier metadata
- [ ] **LORE-02**: `lorebookContext` in `ActionChoicesService` and `SuggestionsService` replaced with same `lorebookEntries[]` variable
- [ ] **LORE-03**: `adventure` and `creative-writing` templates format lorebook entries as `[LOREBOOK CONTEXT]` grouped by type
- [ ] **LORE-04**: `action-choices` and `suggestions` templates format lorebook entries as flat list (`## World Context`)
- [ ] **LORE-05**: `retrievedChapterContext` (combined context passed to EntryInjector) decomposed — lorebook portion passed as structured data; agentic retrieval result passed separately (as a string variable `agenticRetrievalContext` — it is LLM-generated Q&A, harder to structure)

### Chapter History

- [ ] **CHAP-01**: `buildChapterSummariesBlock()` deleted from NarrativeService
- [ ] **CHAP-02**: `chapters[]` array passed to context with fields: number, title, summary, startTime, endTime, characters[], locations[], emotionalTone
- [ ] **CHAP-03**: `timelineFill[]` array passed to context (from TimelineFillResult.responses): chapterNumbers[], query, answer
- [ ] **CHAP-04**: `adventure`, `creative-writing`, and `memory` templates format `<story_history>` block using `{% for chapter in chapters %}` and `{% for item in timelineFill %}`

### Style Guidance

- [ ] **STYLE-01**: `StyleReviewerService.formatForPromptInjection()` no longer used for template injection
- [ ] **STYLE-02**: `styleOverusedPhrases[]` string array passed to context (from StyleReviewResult.phrases)
- [ ] **STYLE-03**: `adventure`, `creative-writing`, and `action-choices` templates format the style avoidance section using `{% for phrase in styleOverusedPhrases %}`
...
| WSC-02 | Phase 2: World State Context | Pending |
| WSC-03 | Phase 2: World State Context | Pending |
| WSC-04 | Phase 2: World State Context | Pending |
| WSC-05 | Phase 2: World State Context | Pending |
| LORE-01 | Phase 3: Lorebook and Style | Pending |
| LORE-02 | Phase 3: Lorebook and Style | Pending |
| LORE-03 | Phase 3: Lorebook and Style | Pending |
| LORE-04 | Phase 3: Lorebook and Style | Pending |
| LORE-05 | Phase 3: Lorebook and Style | Pending |
| STYLE-01 | Phase 3: Lorebook and Style | Pending |
| STYLE-02 | Phase 3: Lorebook and Style | Pending |
| STYLE-03 | Phase 3: Lorebook and Style | Pending |
| CHAP-01 | Phase 4: Chapters and Feature Instructions | Pending |
| CHAP-02 | Phase 4: Chapters and Feature Instructions | Pending |
| CHAP-03 | Phase 4: Chapters and Feature Instructions | Pending |
| CHAP-04 | Phase 4: Chapters and Feature Instructions | Pending |
