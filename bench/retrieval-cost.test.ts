...
import { runRetrieval, type QueryAll } from '@/lib/retrieval'
 ...
 */
const MODEL_ID = 'bench-model'
const BRANCH = 'br_bench'

type Scale = { label: string; happenings: number; awareness: number; chapters: number }
...
function topical(centroids: Float32Array[], topic: number, rand: () => number, mix: number) {
...
function build(
  ...
  return { sqlite, sceneCharacterIds, centroids, dir }
}
