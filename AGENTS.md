...
The following files inside `gen/android/` are **not tracked** (regenerated at build time or by `tauri icon`):
...
- `app/src/main/res/mipmap-*/` — app icons (regenerated by `npx tauri icon`)
- `app/tauri.build.gradle.kts`, `tauri.properties`, `proguard-tauri.pro` — Tauri build config
- `tauri.settings.gradle` — Tauri settings injection
...
npx tauri icon sun2022.png  # Regenerate app icons (mipmap-*)
```
